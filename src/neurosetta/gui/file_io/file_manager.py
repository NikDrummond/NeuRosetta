...
from ...io import export_swc, import_swc, load, save
from ..config import FILE_CONSTANTS
from ..utils.helpers import extract_coordinates_from_csv, validate_csv_data
...
class FileManager:
    ...
    def _load_neuron_file(self, filepath: str) -> tuple[Any, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            logging.info(
                f"Loaded {file_ext.upper().lstrip('.')} file with {len(vertex_coords)} vertices"
            )
    ...
    def save_neuron(self, neuron: Any, filepath: str) -> None:
        ...
        try:
            file_ext = pathlib.Path(filepath).suffix.lower()
            if file_ext == ".nr":
                save(neuron, filepath)
            elif file_ext == ".swc":
                export_swc(neuron, filepath)
            else:
