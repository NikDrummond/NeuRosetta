...
class MainWindow(QMainWindow):
    ...
    def _setup_overlays(self) -> None:
        ...
        self.prev_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_ArrowLeft))
    ...
    def _setup_menus(self) -> None:
        ...
        tools_menu.addAction(
            QAction("Subtree from Point", self, triggered=self._start_subtree_mode)
        )
    ...
    def eventFilter(self, obj, event) -> bool:
        ...
        if obj is self.vtk_widget:
            ...
            elif event.type() == QEvent.MouseButtonPress and event.modifiers() & Qt.ShiftModifier:
    ...
    def _set_scale_size(self) -> None:
        ...
        size, ok = QInputDialog.getInt(
            self, "Scale Bar Size", f"Enter size in {self.settings.units}:", current_size, 1
        )
        if ok:
    ...
    def _show_jump_to_file_dialog(self) -> None:
        ...
        if is_index_mode:
            ...
            try:
                ...
                QMessageBox.warning(
                    self, "Invalid Input", f"'{text}' is not a valid integer index."
                )
        else:
            # Filename mode
