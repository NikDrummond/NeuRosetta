...
def import_swc(fpath: str, ID:str|None = None, metadata:dict|None=None) -> "Tree":
    ...
    meta : _type_, optional
        Optional additional metadata to bind with Tree, by default None
    ...
    # generate neuron
    N = Tree(ID = ID, metadata = meta, graph = graph)

    return N


def write_swc(tree: _Tree, fpath: str, header=None) -> None:
    """Write NeuRosetta.Tree to swc file
