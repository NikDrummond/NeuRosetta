...
from numpy import ndarray, where, unique, concatenate, insert
...
def get_edges(tree, root: int|None = None, subset: str|None = None) -> np.ndarray:

    # get edges
    if root is None:
        edges = tree.graph.get_edges()
    else:
        edges = gt.dfs_iterator(tree.graph,root, array = True)
