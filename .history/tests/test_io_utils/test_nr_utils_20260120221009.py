...
def test_nr_read_write(simple_tree):
    ...
    assert result.metadata['ID'] == 1, "Metadata ID loaded"
    assert result.metadata['units'] == 'undefined', "Metadata Units Loaded"
    assert result.metadata['isReduced'] == False, "Metadata isReduced Loaded"
    assert result.metadata['file_path'] == path, "Metadata file_path Loaded"
